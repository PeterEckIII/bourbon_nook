...
import com.bourbon_nook.bottles_api.models.responses.ErrorResponse;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleBottleNotFoundException(BottleNotFoundException ex) {
        log.warn("Bottle note found: {}", ex.getMessage());
    ...
    @ExceptionHandler(BottleAlreadyExistsException.class)
    public ResponseEntity<ErrorResponse> handleBottleAlreadyExistsException(BottleAlreadyExistsException ex) {
        log.warn("Bottle already exists: {}", ex.getMessage());

        ErrorResponse error = new ErrorResponse.Builder()
                .timestamp(LocalDateTime.now())
                .status(HttpStatus.CONFLICT.value())
                .error(HttpStatus.CONFLICT.getReasonPhrase())
                .errorCode(ErrorCodes.BOTTLE_ALREADY_EXISTS)
                .message(ex.getMessage())
                .developerMessage("Bottle already exists")
                .path(request.getRequestURI())
                .traceId(getTraceId())
                .build();
    ...
    @ExceptionHandler(BottleAlreadyDeletedException.class)
    public ResponseEntity<ErrorResponse> handleBottleAlreadyDeletedException(BottleAlreadyDeletedException ex) {
        log.warn("Bottle already deleted: {}", ex.getMessage());

        ErrorResponse error = new ErrorResponse.Builder()
                .timestamp(LocalDateTime.now())
                .status(HttpStatus.NOT_FOUND.value())
                .error(HttpStatus.NOT_FOUND.getReasonPhrase())
                .errorCode(ErrorCodes.BOTTLE_ALREADY_DELETED)
                .message(ex.getMessage())
                .developerMessage("Bottle with that ID has already been deleted from the system.")
                .path(request.getRequestURI())
                .traceId(getTraceId())
                .build();
