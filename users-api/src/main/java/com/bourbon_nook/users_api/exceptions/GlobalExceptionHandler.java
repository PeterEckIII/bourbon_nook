...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleUserEmailAlreadyExistsException(UserEmailAlreadyExistsException ex) {
        log.warn("User email already exists: {}", ex.getMessage());
        ...
        ErrorResponse error = new ErrorResponse.Builder()
                ...
                .message(ex.getMessage())
                .developerMessage("User email already exists")
                .path(request.getRequestURI())
                .traceId(getTraceId())
                .build();
    ...
    public ResponseEntity<ErrorResponse> handleUserUsernameAlreadyExistsException(UserUsernameAlreadyExistsException ex) {
        log.warn("User username already exists: {}", ex.getMessage());

        ErrorResponse error = new ErrorResponse.Builder()
                .timestamp(LocalDateTime.now())
                .status(HttpStatus.CONFLICT.value())
                .error(HttpStatus.CONFLICT.getReasonPhrase())
                .errorCode(ErrorCodes.USER_USERNAME_ALREADY_EXISTS)
                .message(ex.getMessage())
                .developerMessage("User username already exists")
                .path(request.getRequestURI())
                .traceId(getTraceId())
                .build();
