...
1. **S3 client eagerness — verified, resolved**: `bottles-api`'s `BottleServiceImpl` only calls `S3Template.upload(...)` inside `uploadImage()`, invoked on-demand, not at construction/boot; no e2e spec exercises file upload. Dummy placeholder S3 credentials are safe. Separately: `bottles-api/.env.example` is stale relative to `application.properties` — it lists `ACCESS_KEY`/`SECRET_KEY`/`S3_ENDPOINT`, but the app actually reads `AWS_ACCESS_KEY`/`AWS_SECRET_KEY` (`S3_ENDPOINT` isn't referenced anywhere). The CI `.env` generation step must use the real property names; `.env.example` itself is unchanged by this project since fixing it is unrelated to e2e-in-CI, but worth a follow-up.
